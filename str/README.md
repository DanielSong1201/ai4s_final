...
- `data/processed/<split_name>/pdbbind_sequence_cluster_splits.csv` 是从原始 PDBbind 结构和指定 split 生成的桥接表，包含结构路径、标签、split 等信息。
...
`IID` 可以直接在 `str/scripts/split_raw_pdbbind.sh` 顶部修改，也可以通过环境变量或命令行参数覆盖。优先级为：

```text
--iid 参数 > IID 环境变量 > bash 文件中的默认值
```

README 的默认运行命令不传 `--iid`，因此会使用脚本默认值 `IID=false`：

```bash
PYTHONPATH=$(pwd) bash str/scripts/split_raw_pdbbind.sh
```

如果要临时切换为 IID/random split，推荐显式传参：

```bash
PYTHONPATH=$(pwd) bash str/scripts/split_raw_pdbbind.sh --iid true
```

`SPLIT_NAME` 控制 `data/processed/` 下的输出目录名。默认值为 `split`，因此默认会生成：

```text
data/processed/split/pdbbind_sequence_cluster_splits.csv
```

`SPLIT_NAME` 同样支持脚本默认值、环境变量和命令行参数，优先级为：

```text
--split-name 参数 > SPLIT_NAME 环境变量 > bash 文件中的默认值
```

`SPLIT_NAME` 只能是单层目录名，例如 `split`、`all_raw`、`iid_all_raw`，不能包含 `/`。例如：
...
为了复用后续 manifest 生成脚本，IID 模式下的桥接表也命名为 `pdbbind_sequence_cluster_splits.csv`。这里的文件名是为了兼容接口，不表示 IID 模式做了 sequence-cluster 约束。若希望同时保留多个切分版本，应为每个版本指定不同的 `--split-name`。
...
PYTHONPATH=$(pwd) bash str/scripts/split_raw_pdbbind.sh \
  --iid false \
  --split-name split \
  --output-split-dir str/split_sequence_cluster_all_raw \
```
...
PYTHONPATH=$(pwd) bash str/scripts/split_raw_pdbbind.sh \
  --iid true \
  --split-name iid_all_raw \
  --output-split-dir str/split_iid_all_raw \
  --seed 42
```
...
`build_manifest_from_split.sh` 会根据 `SPLIT_NAME` 自动推导 `SOURCE_CSV`。例如 `--split-name all_raw` 会读取：

```text
data/processed/all_raw/pdbbind_sequence_cluster_splits.csv
```

如果显式提供 `--source-csv` 或 `SOURCE_CSV`，则以显式 CSV 路径为准。
